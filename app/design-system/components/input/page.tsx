...
const InputVariantColumn = ({ title, size }: { title: string; size: "md" | "lg" | "xl" | undefined }): React.JSX.Element => (
  <div className="text-lg text-black font-medium lowercase flex flex-col gap-2 mb-10 grow h-full">
    ...
    <div className="flex flex-col items-start gap-y-[1rem] w-full h-full p-6 py-8 bg-surface-secondary/30 rounded-md border border-stroke-tertiary">
      ...
      <Input size={size} label="Custom styles" className="border border-purple-500 bg-baby-blue-100/20" />
  ...
  return (
    <ComponentDocsPageTemplate title="Input" githubLink={"https://github.com/src/components/ui/Component.tsx"} path="/input">
      <div className="p-8 flex flex-row gap-[1rem] w-full border-stroke-tertiary shadow-sm rounded-md items-stretch">
        ...
        <Input label="Controlled Input" value={inputValue} onChange={(e) => setInputValue(e.target.value)} />
      </div>
