...
type allowedVariants = "default" | "success" | "warning" | "error" | null | undefined;

const AlertVariantBox = ({ heading, title, icon }: { heading: string; title?: string; icon?: IconType }): React.ReactElement | null => (
  <div className="text-lg text-black font-medium flex flex-col gap-2 mb-10">
    <div className="w-[12rem] px-2 text-text-secondary uppercase">{heading}</div>
    <div className="grid grid-cols-1 gap-x-4 gap-y-4 p-6 py-8 bg-white border-stroke-primary shadow-sm border rounded-md place-items-center overflow-x-auto">
      {alertVariants.map((variant, idx) => (
        <Alert
          key={idx}
          icon={icon}
          variant={variant as allowedVariants}
          title={title}
...
const AlertPage = () => (
  <ComponentDocsPageTemplate title="Alert" githubLink={"https://github.com/src/components/ui/Component.tsx"} path="/alert">
    <div className="p-8 flex flex-row gap-x-4">
      <AlertVariantBox heading="Regular" title={"Woah there cowboy!"} />
      <AlertVariantBox heading="With Icon" icon={Warning} title={"Nice looking icon eh"} />
