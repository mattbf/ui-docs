  ...
  if (!bgString)
    return (
      <div className={`flex flex-col   gap-1 flex-1 ${opacity}`}>
        <div
          className={`border border-stroke-secondary rounded-sm min-w-[32px] h-[40px] ${bgString}`}
        />
        <div className="w-full text-center px-1 font-mono text-xs text-black-aplha-500">
          {code}
        </div>
      </div>
    );
  return (
    <TooltipProvider>
      <Tooltip delayDuration={75}>
        <TooltipTrigger>
          <div
            className={`flex flex-col   gap-1 flex-1 ${opacity}`}
            onClick={() =>
              bgString ? void copyToClipboard(bgString, true) : null
            }
          >
            <div
  ...
  <div>
    <div className="mb-8">
      ...
      <div className="mt-2 w-fit cursor-pointer flex h-[1.5rem] p-1 gap-x-1 text-sm items-center text-baby-blue-700 hover:underline transition-colors hover:opacity-90 outline-none select-none">
        ...
        <Link
          href={
            'https://www.figma.com/file/GApUKsrij7hTGqMOPC8wj8/Design-System?type=design&node-id=14%3A2&mode=design&t=3M4nY8yTq6L02OIh-1'
          }
          target="_blank"
        >
          Edit on Figma
    ...
    <ColorRow
      name="graphite"
      colorArray={[25, 50, 75, 100, 200, 500, 600, 700, 800, 900, 950]}
    />
    <ColorRow
      name="evergreen"
      colorArray={[25, 50, 75, 100, 200, 500, 600, 700, 800, 900, 950]}
