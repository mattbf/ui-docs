  ...
  "peer px-[0.125rem] inline-flex  shrink-0 cursor-pointer items-center rounded-full border-[0.5px] border-stroke-secondary transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 focus-visible:ring-offset-background disabled:cursor-not-allowed disabled:opacity-50 data-[state=unchecked]:bg-graphite-75",
  {
    variants: {
      ...
      color: {
        ...
        green: "data-[state=checked]:bg-green-700",
      },
    },
    defaultVariants: {
      color: "black",
...
type SwitchProps = { size?: string } & VariantProps<typeof switchVariants> & React.ComponentPropsWithoutRef<typeof SwitchPrimitives.Root>;

const Switch = React.forwardRef<React.ElementRef<typeof SwitchPrimitives.Root>, SwitchProps>(({ size, color, className, ...props }, ref) => (
  <SwitchPrimitives.Root className={cn(switchVariants({ size, color, className }))} {...props} ref={ref}>
    <SwitchPrimitives.Thumb className={cn(switchThumbVariants({ size, className }))} />
