...
const toastVariants = cva("bg-white rounded-lg p-4 min-w-[20.1875rem] p-4 items-center border-[1px] border-stroke-primary shadow-xl transition-all", {
  variants: {
    variant: {
      default: "",
      success: "",
      warning: "",
      error: "border-[2px] border-red-700",
    },
    position: {
      "top-left":
...
const Toast = React.forwardRef<React.ElementRef<typeof ToastPrimitive.Root>, ToastProps>(
  ({ className, variant, position, icon: Icon, children, ...props }, ref) => (
    <ToastPrimitive.Provider swipeDirection={getSwipeDirection(position ?? "bottom-right")}>
      <ToastPrimitive.Root
        ref={ref}
        className={cn(
          toastVariants({
      ...
      >
        <div className="flex flex-row items-center justify-between gap-x-2">
          {Icon && <Icon size={22} weight="fill" className={cn(VARIANT_TO_ICON_COLOR[variant as string], "shrink-0")} />}
          <div className="w-full flex flex-col justify-center items-start min-w-0">{children}</div>
      ...
      <ToastPrimitive.Viewport
        ...
        style={position?.includes("center") ? { transform: "translate(-50%)" } : undefined}
      />
...
const ToastRoot = React.forwardRef<React.ElementRef<typeof ToastPrimitive.Root>, ToastProps>(
  ({ className, variant, position, icon: Icon, children, ...props }, ref) => (
    <ToastPrimitive.Root
      ref={ref}
      className={cn(
...
const ToasterWrapper = ({ position, children }: React.PropsWithChildren<{ position?: Position }>) => (
  <ToastPrimitive.Provider swipeDirection={getSwipeDirection(position ?? "bottom-right")}>
    {children}
    <ToastPrimitive.Viewport
      className={cn(
...
const ToastTitle = ({ className, title, ...props }: React.HTMLAttributes<HTMLDivElement>) => (
